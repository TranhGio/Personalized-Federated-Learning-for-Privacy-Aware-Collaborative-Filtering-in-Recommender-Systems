...
def test_canonical_artifact_carries_best_and_last_blocks(tmp_path) -> None:
    ...
    mf = loaded["_manifest"]
    # Phase 7 Plan 01 D-22: bumped schema 2->3 (added thesis_run_label / ablation_dimension / ablation_value).
    # Phase 6 D-07 invariants (final_eval_round_index + metrics) preserved as v2 carry-forward.
    assert mf["schema_version"] == 3, f"_manifest.schema_version must be 3, got {mf['schema_version']}"
...
def _server_app_src() -> str:
    ...
    src_path = Path(__file__).resolve().parents[1] / "federated_baseline_cf" / "server_app.py"
