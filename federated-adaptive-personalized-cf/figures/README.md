...
- Do NOT confuse the Dirichlet concentration **α** (partitioning heterogeneity) with the adaptive personalization **α** computed by `adaptive_alpha.py` for this module — they are unrelated despite sharing the symbol.
- For cross-device diagnostics (per-user interaction histograms, sparse/medium/dense bucket distribution, per-user learned α distributions, item-perturbation norms), see the eval-harness outputs under `../../results/federated/adaptive/<run_id>/` once Phase 6 lands, plus `../federated_adaptive_personalized_cf/evaluation/alpha_analysis.py`.
