"""Unit tests for PersonalizedSplitFedAvg/PersonalizedSplitFedProx sufficient-stat aggregation (Phase 3 Plan 01)."""
...
def test_personalized_split_fedprox_inherits_aggregate_evaluate(fake_evaluate_res, fake_client_proxy) -> None:
    strategy = PersonalizedSplitFedProx(fraction_fit=0.1, proximal_mu=0.01)
    results = [
...
def test_aggregate_fit_inherited_unchanged() -> None:
    """aggregate_fit must NOT be overridden — split-learning D-23 invariant.

    PersonalizedSplitFedAvg only sends GLOBAL params (item_*); parent FedAvg's
    weighted average of those GLOBAL params is correct for split learning.
    """
    from flwr.server.strategy import FedAvg as _FedAvg
    assert PersonalizedSplitFedAvg.aggregate_fit is _FedAvg.aggregate_fit, (
        "PersonalizedSplitFedAvg MUST inherit aggregate_fit unchanged from FedAvg "
        "(D-23: split-learning aggregates only GLOBAL params; client is the filter)"
    )
