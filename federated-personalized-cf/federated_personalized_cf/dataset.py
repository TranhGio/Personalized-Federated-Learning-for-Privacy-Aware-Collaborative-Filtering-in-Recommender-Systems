"""MovieLens 1M Dataset Loading — Phase 3 thin adapter over fedrec_foundation.

Post-Phase-3 this module is responsible only for:
...
personalized cross-device migration removes multi-user support. Check out a
pre-Phase-3 commit (see ``.planning/phases/03-personalized-migration/03-CONTEXT.md``
§Deferred) to reproduce legacy cross-silo numbers.
...
# --- Phase 3 Plan 02: foundation-backed adapters ---
...
def load_partition_data(
    ...
    Post-Phase-3 callers (``client_app.py::@app.train()``, ``@app.evaluate()``,
    ``task.py::load_data``) pass ``partition_id`` and expect the same 6-tuple
    as before. The ``alpha`` / ``test_ratio`` / ``split_mode`` parameters
    are retained for backwards compatibility but under the benchmark path
    ...
    if partition_mode == "dirichlet":
        raise NotImplementedError(
            "Personalized cross-device migration removed multi-user support per D-02. "
            "Check out a pre-Phase-3 commit (see "
    ...
    if split_mode != "leave-one-out":
        raise ValueError(
            f"split_mode={split_mode!r} not supported post-Phase-3 foundation "
