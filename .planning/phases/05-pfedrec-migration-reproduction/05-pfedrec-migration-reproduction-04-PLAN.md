...
must_haves:
  truths:
    ...
    - "D-15 manifest double-write with module='pfedrec' (PFR-09); D-27 best-round-restore against sampled_ndcg@10 (D-13); FitRes.num_examples = 1 per client (D-24 uniform via Pitfall 5 Option B)"
  ...
  artifacts:
    - path: "federated-pfedrec/federated_pfedrec/server_app.py"
      provides: "Cross-device PFedRec main loop with D-14 PFR-08 auto-verify hook + D-15 manifest double-write + Phase-3-template carry-forward"
      contains: "[PFR-08 VERIFIED]"
    - path: "federated-pfedrec/tests/test_server_integration.py"
      provides: "8 GREEN integration tests covering G-03-01 / PFR-06 / D-14 (3 sub-tests) / D-15 / D-13 / D-27"
  key_links:
    ...
    - from: "server_app.py D-15 manifest"
      to: "results_data['_manifest']['module']='pfedrec' + sibling <run_id>-manifest.json"
...
Purpose:
  ...
  - PFR-09: D-15 double-write manifest with `module="pfedrec"` (Phase 3 / Phase 4 idiom).

Output:
  - 1 modified file (server_app.py — full cross-device main loop).
  - 1 new test file (test_server_integration.py — 8 GREEN integration tests).
</objective>
...
def main(grid: Grid, context: Context):
    ...
    # 9. FL loop with D-13 cold-start + D-27 best-round
    ...
    for round_num in range(1, num_rounds + 1):
        ...
        # D-27 best-round snapshot (D-13 against sampled_ndcg@10)
...
<task type="auto">
  <name>Task 1: Rewrite server_app.py with full Phase-5 cross-device main loop including the D-14 PFR-08 auto-verify hook</name>
  <files>federated-pfedrec/federated_pfedrec/server_app.py</files>
  <read_first>
    ...
    - federated-personalized-cf/federated_personalized_cf/server_app.py — Phase 3 Plan 4 server_app reference (D-13 cold-start counter pattern)
    ...
    - .planning/phases/05-pfedrec-migration-reproduction/05-CONTEXT.md §decisions D-02, D-09, D-10, D-13, D-14, D-15, D-24, D-26, D-27
    ...
    - scripts/foundation/fedrec_foundation/manifest.py — build_run_manifest signature + RunManifest fields
    - scripts/foundation/fedrec_foundation/rng.py — server_rng signature + .sample(range, k) API
  </read_first>
  <action>

Rip-and-replace `federated-pfedrec/federated_pfedrec/server_app.py` cloning the Phase 4 Plan 5 server template with the 5 PFedRec-specific deltas listed in RESEARCH §Pattern 2:
...
**Delta 5: D-14 PFR-08 auto-verify hook** — NEW to Phase 5. Implement two helpers exactly as shown in the `<interfaces>` block above:
- `_parse_reference_results(reference_path: Path) -> Tuple[float, float]` — reads `IJCAI-23-PFedRec/sh_result/ml-1m.txt`, picks `lines[-1]` (line 2 today: HR=0.7286, NDCG=0.4407 — Open Question 1 recommendation: most-recent / closest-to-paper-round-89 target).
- `_emit_pfr_08_verification(final_metrics, reference_path, tolerance_pts=2.0) -> Tuple[bool, str, Dict]` — compares to ±2.0 absolute percentage points (multiplying ratios by 100 for log readability), returns (passed, log_line, audit_dict). NON-FATAL — failed reproduction does NOT raise.

The hook fires AFTER `embed_manifest_in_result(manifest, results_data)` and BEFORE the W&B summary write (RESEARCH Open Question 3 recommendation):

```python
embed_manifest_in_result(manifest, results_data)

# D-14 PFR-08 auto-verify hook (Phase 5 NEW)
reference_path = Path(__file__).resolve().parents[2] / "IJCAI-23-PFedRec" / "sh_result" / "ml-1m.txt"
pfr08_passed, pfr08_log_line, pfr08_audit = _emit_pfr_08_verification(
    final_metrics=final_metrics,
...
print(pfr08_log_line)
# Post-embed mutation pattern (Phase 4 Plan 5 precedent — embed_manifest_in_result returns same dict by reference)
...
- D-13 cold-start counter: probe `.embedding_cache/{run_id}/partition_{pid}.pt` existence per selected pid; results_data["cold_starts"] = {per_round, total, rate}; under reuse_cache=true short-circuit to 0.
- D-27 best-round snapshot: `if current_ndcg > best_metric:` snapshot ArrayRecord with deep-clone; restore at loop end before final result write; checkpoint_rule accepts both 'best_round_restore' and 'best_round' spellings.
- D-15 manifest double-write: `build_run_manifest(..., module="pfedrec")` + `embed_manifest_in_result` + `write_manifest_sibling`.
...
After writing the file, run a smoke import check: `python -c "from federated_pfedrec.server_app import app, _parse_reference_results, _emit_pfr_08_verification; print('ok')"`. Should print `ok` without crashing.
  </action>
  <verify>
    <automated>cd /home/bes/Desktop/vinh/federated-learning/movie-recommendation-system && python -c "from federated_pfedrec.server_app import app, _parse_reference_results, _emit_pfr_08_verification; print('ok')"</automated>
  </verify>
  <acceptance_criteria>
    ...
    - `grep -c "best_round_restore\|best_round" federated-pfedrec/federated_pfedrec/server_app.py` returns at least 2 (D-27 + spelling tolerance)
    - `grep -c "cold_starts\|cold_count" federated-pfedrec/federated_pfedrec/server_app.py` returns at least 3 (D-13 counter)
  ...
  <done>
    - server_app.py: full cross-device migration with all 5 PFedRec-specific deltas + Phase 3/4 carry-forward
    ...
    - PFR-09: D-15 double-write manifest with `module="pfedrec"`
    - stdlib random eradicated; smoke import GREEN
  </done>
</task>

<task type="auto" tdd="true">
  <name>Task 2: Ship test_server_integration.py with 8 GREEN integration tests covering G-03-01 + PFR-06 + D-14 (3 sub-tests including parse / pass / fail) + D-15 + D-13 + D-27</name>
  <files>federated-pfedrec/tests/test_server_integration.py</files>
  <read_first>
    - federated-pfedrec/federated_pfedrec/server_app.py (post-Task 1) — for the helpers + main loop shape
    ...
    - IJCAI-23-PFedRec/sh_result/ml-1m.txt — for test_pfr08_autoverify_parses_sh_result
  </read_first>
  <behavior>
    ...
    - Test 3 (test_pfr08_autoverify_parses_sh_result): import `_parse_reference_results` from `federated_pfedrec.server_app`; call with the actual `IJCAI-23-PFedRec/sh_result/ml-1m.txt` path; assert returns `(0.7286423841059603, 0.4407401988138434)` (line 2 — most recent — Open Question 1 recommendation). Allow `pytest.approx(0.7286, abs=1e-3)` tolerance for float robustness.
    ...
    - Test 8 (test_best_round_restore_against_ndcg10): grep server_app.py source for `current_ndcg = thesis_metrics.get("sampled_ndcg@10"`; assert match. Assert source contains `if checkpoint_rule in ("best_round_restore", "best_round")` (D-27 + spelling tolerance). Assert source contains `arrays = best_arrays` (D-27 restore). Use `src.rfind(...)` for the restore string if proximity-aware (Phase 4 Plan 5 lesson — module docstring may duplicate the literal).
  </behavior>
  <action>

Create `federated-pfedrec/tests/test_server_integration.py` with 8 tests covering VALIDATION.md rows 5-04-01 through 5-04-08. Follow Phase 3 / Phase 4 idiom: source-level proximity tests where a live Grid is unavailable; use `inspect.getsource(server_app)` to read the source as text.

Test 4 + Test 5 — the auto-verify pass/fail tests — instantiate `_emit_pfr_08_verification` with synthetic reference files via `tmp_path`. Sketch:

```python
def test_pfr08_autoverify_pass_within_2pts(tmp_path) -> None:
    """D-14: auto-verify returns passed=True when within ±2 pts of reference."""
    from federated_pfedrec.server_app import _emit_pfr_08_verification

    # Synthetic reference file with one line; same dash-separated format as ml-1m.txt
...
def test_pfr08_autoverify_fail_outside_2pts(tmp_path) -> None:
    """D-14: auto-verify returns passed=False without raising on out-of-tolerance."""
    from federated_pfedrec.server_app import _emit_pfr_08_verification
...
Test 3 — anchor on the actual reference file:

```python
def test_pfr08_autoverify_parses_sh_result() -> None:
    """D-14: parse the actual IJCAI-23-PFedRec/sh_result/ml-1m.txt; pick line 2 (most recent)."""
    from pathlib import Path
...
For Tests 1, 2, 6, 7, 8 — use `inspect.getsource(server_app_module)` source-level proximity checks (Phase 3/4 idiom). For Test 8 specifically, use `src.rfind(...)` to anchor on the LAST occurrence of the restore string (avoid docstring-duplicate false-positive — Phase 4 Plan 5 lesson).

Run: `cd federated-pfedrec && pytest tests/test_server_integration.py -x -v` → 8 GREEN.
  </action>
  <verify>
  ...
  <done>
    ...
    - D-14 PFR-08 auto-verify hook locked at the test layer: parse / pass / fail path all asserted
    - D-15 module='pfedrec' manifest double-write asserted at source level
    - D-13 cold-start counter, D-27 best-round-restore, G-03-01 discovery + ADP-06 sampling all asserted at source level
  </done>
</task>
...
<success_criteria>
- server_app.py: full Phase-5 cross-device main loop with all 5 PFedRec-specific deltas (D-12 strategy, D-01 GLOBAL bias, no prototype/alpha, no centralized eval, D-14 auto-verify) + Phase 3/4 carry-forward (D-25 mode resolver, D-02 mirror, G-03-01 discovery, ADP-06 sampler, D-13 cold-start, D-27 best-round, D-15 double-write, Pitfall 5 Option B for D-24 uniform)
- 8 GREEN tests (test_server_integration.py) cover VALIDATION rows 5-04-01..08
...
- 5 PFedRec-specific deltas + Phase 3/4 carry-forward
...
- Test counts mapped to VALIDATION rows 5-04-01..08
- Plan 05 readiness: subprocess regression guard can consume selected_clients_per_round + .embedding_cache schema_v3 cache + _manifest.pfr08_verification field
</output>
