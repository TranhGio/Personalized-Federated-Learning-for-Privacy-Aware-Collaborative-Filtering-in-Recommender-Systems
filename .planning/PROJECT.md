...
- [ ] All four Flower modules run with `num-supernodes = 6040` and `partition-mode = "natural"` (1 user = 1 client) as the primary configuration
...
- [ ] Test-positive-leaks-into-training-negatives bug fixed in all four modules (`user_rated_items` must include the held-out test item)
...
- [ ] Server-level seed set so per-round client selection and evaluation negative sampling are reproducible
