...
- ✓ Evaluation & Reporting Harness (Phase 6) — EVL-01..06: foundation `module_run_results_dir(module, run_id)` helper at `scripts/foundation/fedrec_foundation/paths.py:59` (D-01/D-02 repo-root anchor, `_ALLOWED_MODULES` whitelist), RunManifest schema bumped v1→v2 with `final_eval_round_index` + `metrics` safe-default fields and `write_manifest_sibling(..., sibling_name=None)` kwarg (D-04, D-07), all 4 `server_app.py` files wired with D-06 extra-eval-round (broadcast eval over restored best-round arrays after `arrays = best_arrays`; replaces D-06-forbidden `eval_metrics_history[best_round_num]` lookup), nested `final_metrics = {best, last, best_round, last_round, final_eval_round_index}` (D-07), Pitfall-9 `last_round = max(eval_metrics_history.keys())`, W&B `best/*` + `last/*` namespaces replacing legacy `final/*`, `dataclass_replace(manifest, final_eval_round_index=N, metrics=...)` before `embed_manifest_in_result`, np.float64 JSON-safe coercion across all 4 modules, cross-silo legacy path preserved (D-03 + Pitfall-8); Pitfall-1 closed in PFedRec (`_emit_pfr_08_verification(final_metrics["best"], ...)` consumes nested-best block); Pitfall-4 closed in adaptive (extra-eval ConfigRecord carries restored `best_prototype`); `sweep.yaml` `metric.name` migrated `final/sampled_ndcg@10` → `best/sampled_ndcg@10` (Pitfall-7); D-09 per-round exposure-history regression guards strengthened across all 4 module test suites with `evaluated_users` (overall) + per-group keys; `test_wandb_summary_keys.py` (yaml.safe_load structured parse). 278 GREEN tests across all suites (foundation 100 + baseline 26 + personalized 38 + adaptive 73 + pfedrec 41), zero regressions. EVL-01..06 satisfied. Three live-run UAT items pending in `06-HUMAN-UAT.md`: W&B project routing, cross-silo path coexistence, full PFedRec paper_compat 100-round reproduction.
...
- [x] Early stopping checkpoints best-round parameters instead of reporting last-round metrics — DONE in Phase 6 (EVL-01): D-06 extra-eval-round broadcasts evaluation over restored best-round arrays in all 4 modules; canonical `final_metrics["best"]` is the broadcast result, not a stale history lookup; `final_eval_round_index` flows into the manifest
...
- [ ] Results exported to `results/federated/` with full experiment metadata and logged to a dedicated **cross-device** W&B project (separate from existing cross-silo runs) — code DONE in Phase 6 (EVL-04 + EVL-05): cross-device runs write to `results/federated/<module>/<run_id>/results.json` + `manifest.json`, cross-silo legacy flat layout preserved; live W&B routing pending UAT confirmation in `06-HUMAN-UAT.md`
...
---
*Last updated: 2026-04-29 after Phase 6 (Evaluation & Reporting Harness) completion — EVL-01..06 shipped across all 4 modules. Foundation primitives (`module_run_results_dir`, RunManifest schema v2 with `final_eval_round_index` + `metrics` + `sibling_name` kwarg) in place; every server_app rewired to (a) run an extra eval round over restored best-round arrays (no more stale history lookup), (b) emit nested `final_metrics = {best, last, best_round, last_round, final_eval_round_index}`, (c) split W&B summary into `best/*` + `last/*` namespaces, (d) write to per-run-dir `results/federated/<module>/<run_id>/{results.json, manifest.json}` while preserving cross-silo legacy paths. Pitfall-1 (PFedRec D-14 PFR-08 hook → `final_metrics["best"]`), Pitfall-4 (adaptive `best_prototype` on extra-eval ConfigRecord), Pitfall-7 (sweep.yaml `metric.name` → `best/sampled_ndcg@10`) all closed. 278 GREEN tests across all suites (foundation + baseline + personalized + adaptive + pfedrec), zero regressions. Three live-run items pending in `06-HUMAN-UAT.md`: W&B dashboard routing, cross-silo path coexistence, full PFedRec paper_compat 100-round reproduction. Phase 7 (Thesis Evaluation Run) is next — the actual cross-module comparison and the thesis-contribution headline number.*
